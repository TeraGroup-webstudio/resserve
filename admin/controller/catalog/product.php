...
class ControllerCatalogProduct extends Controller {
	...
	protected function getForm() {
        ...
        // Stickers
        if (isset($this->request->post['product_sticker'])) {
            $product_stickers = $this->request->post['product_sticker'];
        } elseif (isset($this->request->get['product_id'])) {
            $product_stickers = $this->model_catalog_product->getProductStickers($this->request->get['product_id']);
        } else {
        ...
        foreach ($product_stickers as $product_sticker) {
            if (is_file(DIR_IMAGE . $product_sticker['sticker'])) {
                $sticker = $product_sticker['sticker'];
                $thumb = $product_sticker['sticker'];
            } else {
                $sticker = '';
                $thumb = 'no_image.png';
            }
            ...
            $data['product_stickers'][] = array(
                'sticker'      => $sticker,
                'thumb'      => $this->model_tool_image->resize($thumb, 100, 100),
                'sort_order' => $product_sticker['sort_order']
            );
        }
