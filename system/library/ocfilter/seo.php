...
class Seo extends Factory {
  ...
  public function rewrite($link) {
    ...
    if (isset($data['ocf_product'])) {
      unset($data['ocf_product']);
    ...
    if (isset($data['ocf_product_category'])) {
      $category_id = $data['ocf_product_category'];

      unset($data['ocf_product_category']);
    }
    ...
    if (isset($data[$this->params->getIndex()]) && $category_id) {
      $page_info = $this->opencart->model_extension_module_ocfilter->getPageByParams($category_id, $this->params->decode($data[$this->params->getIndex()]));
    ...
    if ($this->isSeoUrlEnabled()) {
      ...
      if ($product_link) {
        array_splice($parts, 1, -1, $page_info['keyword']);
      } else {
        $parts[] = $page_info['keyword'];
      }

      // Set URL keyword and suffix
      $path = implode('/', $parts) . $this->getUrlSuffix($url);
    }
  ...
  public function getPageBreadcrumb() {
    ...
    if ($page_info) {
      $name = $page_info['name'];
    } else if (!$name && $this->getProductId() && $this->getCategoryId()) {
      $query = $this->opencart->db->query("SELECT manufacturer_id FROM `" . DB_PREFIX . "product` WHERE product_id = '" . (int)$this->getProductId() . "'");

      if (!$query->num_rows) {
        return false;
      }

      $params = 'F1S0V' . $query->row['manufacturer_id'];

      $page_query = $this->opencart->db->query("SELECT opd.* FROM `" . DB_PREFIX . "ocfilter_page` op LEFT JOIN `" . DB_PREFIX . "ocfilter_page_description` opd ON (op.page_id = opd.page_id) WHERE op.category_id = '" . (int)$this->getCategoryId() . "' AND op.`params` = '{\"1.0\":[\"" . (int)$query->row['manufacturer_id'] . "\"]}' AND opd.language_id = '" . (int)$this->opencart->config->get('config_language_id') . "'");

      if ($page_query->num_rows) {
        $name = $page_query->row['name'];
      }
    }
