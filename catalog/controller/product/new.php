...
class ControllerProductNew extends Controller {
	public function index() {
		...
		$data['sorts'][] = array(
			...
			'href'  => $this->url->link('product/new', 'sort=p.viewed&order=DESC' . $url)
		);
        ...
        $data['sorts'][] = array(
            ...
            'value' => 'p.viewed-ASC',
            'href'  => $this->url->link('product/new', '&sort=p.viewed&order=ASC' . $url)
        );
		...
		foreach($limits as $value) {
			$data['limits'][] = array(
				'text'  => $value,
				'value' => $value,
				'href'  => $this->url->link('product/new', $url . '&limit=' . $value)
			);
		}
		...
		if ($page == 1) {
		    $this->document->addLink($this->url->link('product/new', '', true), 'canonical');
		} else {
		    $this->document->addLink($this->url->link('product/new', 'page='. $page , true), 'canonical');
		}		
		
		if ($page > 1) {
			$this->document->addLink($this->url->link('product/new', (($page - 2) ? '&page='. ($page - 1) : ''), true), 'prev');
		}

		if ($limit && ceil($product_total / $limit) > $page) {
		    $this->document->addLink($this->url->link('product/new', 'page='. ($page + 1), true), 'next');
		}
