...
class ModelExtensionModuleOCFilter extends Model {
  ...
  public function getCounters($data = []) {
    ...
    $cached = !$this->ocfilter->params->hasSlider();
//var_dump($this->ocfilter->config('category_visibility'));
    if ($cached) {
  ...
  public function clonePage($page_id, $category_id, $params) {
            ...
            if ($keyword_query->num_rows && utf8_strlen($keyword_query->row['keyword']) > 0) {
              ...
              foreach ($category_query->rows as $row) {
                if ($language_id == $row['language_id']) {
                  $keyword = str_replace('{category}', $this->ocfilter->helper->translit($row['name']), $keyword);
                  $keyword = str_replace('{category|L}', $this->ocfilter->helper->translit($row['name']), $keyword);
                }
              }
    ...
    foreach ($category_query->rows as $row) {
      foreach ($page_data['description'] as $language_id => $description) {
        if ($language_id == $row['language_id']) {
          // Default
          $description = $set_page_description('{category}', $row['name'], $description);

          // Lowercase
          $description = $set_page_description('{category|L}', utf8_strtolower($row['name']), $description);
  ...
  public function getPageByParams($category_id, $filter_params = []) {
    ...
    if (!$page_data) {
      ...
      $query = $this->ocfilter->query("SELECT page_id, params, dynamic FROM " . DB_PREFIX . "ocfilter_page WHERE status = '1' AND dynamic = '1' AND (category_id = '" . (int)$category_id . "' OR category_id = '0') AND params_count = '" . (int)$params_count . "'");
    ...
    if ($page_data && $page_data['dynamic']) {
      $page_data = $this->getPage($this->clonePage($page_data['page_id'], $category_id, $filter_params));
    }
